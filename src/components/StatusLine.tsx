...
import { checkIndexHealth } from '../utils/indexHealth.js';
...
function formatCacheTokenCount(tokens: number): string {
...
function CachePill({ messages }: CachePillProps): React.ReactNode {
  ...
  const cacheReadText = usage !== null ? formatCacheTokenCount(usage.cache_read_input_tokens) : '--';
  const cacheWriteText = usage !== null ? formatCacheTokenCount(usage.cache_creation_input_tokens) : '--';
  ...
  // Low hit-rate advisory: <50% warning, 50-80% hint
  const hitRateWarning =
    displayHitRate !== null && displayHitRate < 80 ? (displayHitRate < 50 ? ' cache偏低' : ' cache波动') : null;

  return (
    <Text>
      <Text dimColor>{' 缓存 '}</Text>
      <Text color={hitRateColor}>{hitRateText}</Text>
      {hitRateWarning ? <Text dimColor>{hitRateWarning}</Text> : null}
      <Text dimColor>{` R${cacheReadText} W${cacheWriteText}`}</Text>
...
// ---------------------------------------------------------------------------
// IndexHealthPill — index staleness reminder
// ---------------------------------------------------------------------------

function IndexHealthPill(): React.ReactNode {
  const [suggestion, setSuggestion] = useState<string | null>(null);

  useEffect(() => {
    const cwd = getCwd();
    checkIndexHealth(cwd, null, null)
      .then(result => {
  ...
  return <Text color="warning">{` ${suggestion}`}</Text>;
}
