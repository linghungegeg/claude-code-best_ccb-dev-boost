  ...
  return (
    <SentryErrorBoundary name="MessagesBoundary">
      ...
      {streamingText &&
        !isBriefOnly &&
        !((): boolean => {
          // Dedup: skip streaming preview if the last assistant message
          // already contains identical text (race between setMessages and
          // setStreamingText(null) after streaming finishes).
          const last = renderableMessages.at(-1);
          if (!last || last.type !== 'assistant') return false;
          const blocks = last.message?.content as Array<{ type: string; text?: string }> | undefined;
          if (!Array.isArray(blocks)) return false;
          return blocks.some(b => b.type === 'text' && b.text === streamingText);
        })() && (
