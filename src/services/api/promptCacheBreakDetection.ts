...
export async function checkResponseForCacheBreak(
  ...
  try {
    ...
    storeBreakSummary(reason, prevCacheRead, cacheReadTokens, changes, diffPath)
...
// ---------------------------------------------------------------------------
// Public summary for /break-cache status and panel
// ---------------------------------------------------------------------------
...
let lastBreakSummary: CacheBreakSummary | null = null

export function getCacheBreakSummary(): CacheBreakSummary | null {
  return lastBreakSummary
}

/**
 * Stores a summary after a cache break is detected so /break-cache status
 * and the break-cache panel can surface it to the user.
 */
function storeBreakSummary(
  reason: string,
  ...
  lastBreakSummary = {
    ...
    details: changes
      ? {
          ...
          removedTools: changes.removedTools,
          changedToolSchemas: changes.changedToolSchemas,
          systemCharDelta: changes.systemCharDelta,
          betasChanged: changes.betasChanged,
          effortChanged: changes.effortChanged,
          cacheControlChanged: changes.cacheControlChanged,
        }
      : null,
  }
}
