...
If verification fails because this skill's instructions are outdated (dev server command/port/ready-signal changed, etc.) — not because the feature under test is broken — or if the user corrects you mid-run, tell the user what needs updating and why. You cannot edit the project files yourself (the Verify agent has no file-write permission), but you can use AskUserQuestion to confirm the fix with the user, and they can edit the SKILL.md or re-run /init-verifiers to regenerate it.
\`\`\`
