...
export async function callBreakCache(
  ...
  if (scope === 'status') {
    ...
    // Include cache-break reason from promptCacheBreakDetection if available
    const breakSummary = getCacheBreakSummary()
    let breakDetailBlock = ''
    if (breakSummary) {
      const ts = breakSummary.lastBreakAt
        ? new Date(breakSummary.lastBreakAt).toLocaleString('zh-CN')
        : 'unknown'
      breakDetailBlock = [
        '',
        '## 上次缓存破坏',
        `  时间:        ${ts}`,
        `  原因:        ${breakSummary.reason}`,
        breakSummary.prevCacheRead !== null
          ? `  cache read:  ${breakSummary.prevCacheRead} → ${breakSummary.cacheRead}`
          : '',
        breakSummary.diffPath ? `  diff 文件:   ${breakSummary.diffPath}` : '',
        ...(breakSummary.details
          ? [
              `  详情:`,
              breakSummary.details.systemPromptChanged
                ? `    - system prompt 变化`
                : '',
              breakSummary.details.toolSchemasChanged
                ? `    - tools 变化 (+${breakSummary.details.addedTools.length}/-${breakSummary.details.removedTools.length})`
                : '',
              breakSummary.details.addedTools.length > 0
                ? `      新增: ${breakSummary.details.addedTools.join(', ')}`
                : '',
              breakSummary.details.removedTools.length > 0
                ? `      移除: ${breakSummary.details.removedTools.join(', ')}`
                : '',
              breakSummary.details.changedToolSchemas.length > 0
                ? `      schema 变化: ${breakSummary.details.changedToolSchemas.join(', ')}`
                : '',
              breakSummary.details.modelChanged ? `    - model 切换` : '',
              breakSummary.details.betasChanged ? `    - betas 变化` : '',
              breakSummary.details.effortChanged ? `    - effort 变化` : '',
              breakSummary.details.cacheControlChanged
                ? `    - cache_control 变化`
                : '',
            ]
          : []),
