 ...
 *    - Static content before boundary — including attribution header
 *      and CLI prefix when they appear before the boundary marker
 *      (cacheScope='global')
 *    - Dynamic content after boundary (cacheScope=null)
 *    - Attribution/CLI prefix only extracted separately when they
 *      anomalously appear after the boundary (cacheScope=null)
 *
  ...
  if (useGlobalCacheFeature) {
    ...
    if (boundaryIndex !== -1) {
      ...
      for (let i = 0; i < systemPrompt.length; i++) {
        ...
        if (i < boundaryIndex) {
          // Everything before the boundary (including attribution header
          // and CLI prefix) is session-stable — lump it into the globally
          // cached block instead of extracting it to a separate uncached block.
          staticBlocks.push(block)
        } else if (
          block.startsWith('x-anthropic-billing-header') ||
          CLI_SYSPROMPT_PREFIXES.has(block)
        ) {
          // Attribution/CLI prefix appearing AFTER the boundary is
          // anomalous — keep it uncached to be safe.
          if (block.startsWith('x-anthropic-billing-header'))
            attributionHeader = block
          else systemPromptPrefix = block
        } else {
