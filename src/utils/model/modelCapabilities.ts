...
const STATIC_MODEL_CAPABILITIES: ModelCapability[] = sortForMatching([
  { id: 'deepseek-v4-pro', max_input_tokens: 1_000_000 },
  { id: 'deepseek-v4-flash', max_input_tokens: 1_000_000 },
])
...
export function getModelCapability(model: string): ModelCapability | undefined {
  ...
  const staticExact = STATIC_MODEL_CAPABILITIES.find(
    c => c.id.toLowerCase() === m,
  )
  if (staticExact) return staticExact
  const staticMatch = STATIC_MODEL_CAPABILITIES.find(c =>
    m.includes(c.id.toLowerCase()),
  )
