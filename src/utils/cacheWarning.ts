...
export function createCacheWarningMessage(info: CacheHitRateInfo): Message {
  ...
  const rateText = hitRate.toFixed(0)
  const thresholdText = threshold.toString()
  let content = `缓存命中率 ${rateText}%，低于 ${thresholdText}% 阈值`
  ...
  if (hitRate < 50) {
    suggestions.push('命中率严重偏低，建议使用 /compact 压缩对话上下文')
  } else if (hitRate < 70) {
    suggestions.push('建议避免在消息中粘贴大段日志或代码块，可使用文件引用替代')
  }
  if (trend !== null && trend < -5) {
    suggestions.push('命中率持续下降，检查是否频繁切换模型或修改 system prompt')
  }
